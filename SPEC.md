...
**Altered → TMFX (token-level).** When the integration setting `tmfxIntegration` is on (default on if `tokenmagic` module is active) and the Altered AE applies to a token, the substance's TMFX filter params are pushed onto the token. Filter params live on the substance flag block (`flags["substances-and-paraphernalia"].tmfxFilterParams`). On AE removal, filter is popped. No TMFX → silent no-op.
